...
class DefaultController extends BaseController {
    ...
    public function loginAction() {
        ...
        return $this->render(
                        'UNAMAppBundle:Default:login.html.twig', array(
    ...
    /**
     * @Route("/cambio/horario", name="cambio_horario")
     * @Method("POST")
     */
    public function cambioHorarioAction(Request $request) {
        if ($request->getMethod() == 'POST') {
            $data = $request->request->all();
            $em = $this->getDoctrine()->getManager();
            $grupo = $em->getRepository('UNAMAppBundle:Grupo')->find($data['grupoId']);
            if ($grupo == null) {
                ...
                $this->enviarCambioHorarioEmail($grupo, $data['nuevo_horario'], $data['message']);
                $datos = array(
                    'status' => 'Ok',
                    'alumnosCount' => count($grupo->getPagos())
                );
            }
            $response = new JsonResponse($datos);
            return $response;
